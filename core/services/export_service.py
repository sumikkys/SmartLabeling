...
def update_yaml(yaml_path: str, image_id: str) -> str:
    try:
    ...
    try:
        classes = cache_manager.image_class_cache
    except KeyError:
...
def export_annotations(request: ExportRequest) -> Dict[str, str]:
    ...
    for image_id in valid_image_ids:
        try:
            ...
            # 获取数据
            data = cache_manager.image_data_cache[image_id]["masks"]
            image_width = cache_manager.image_data_cache[image_id]["width"]
            image_height = cache_manager.image_data_cache[image_id]["height"]
                    ...
                    for y, row in enumerate(annotation):
                        for x, value in enumerate(row):
                            if value != 0.0 and 0 <= x < image_width and 0 <= y < image_height:
                                mask_data[y, x] = 256 - int(class_id) #反向导出灰度方便调试
...
def export_cache(request: ExportRequest) -> str:
    ...
    data = {
        "image_id_cache": cache_manager.image_id_cache,
